define(["storymaps/utils/Helper"], 
	function (Helper) {
		return function SettingsPopupTabHeader(titleContainer, contentContainer, defaultLogoURL) 
		{
			...
			{			
				...
				if ( ! APPCFG.HEADER_SOCIAL.facebook )
					$(".enableFB", contentContainer)
						...
						.parent()
						.addClass("disabled")
						.attr("title", i18n.builder.settingsHeader.settingsLogoSocialDisabled);
			...
			{
				$(titleContainer).html(i18n.builder.settingsHeader.settingsTabLogo);
				
				$(contentContainer).find('p').eq(0).html(i18n.builder.settingsHeader.settingsLogoExplain);
				$(contentContainer).find('.logoText').eq(0).html(i18n.builder.settingsHeader.settingsLogoEsri);
				$(contentContainer).find('.logoText').eq(1).html(i18n.builder.settingsHeader.settingsLogoNone);
				$(contentContainer).find('.logoText').eq(2).html(i18n.builder.settingsHeader.settingsLogoCustom);
				$(contentContainer).find('#logoInput').attr("placeholder", i18n.builder.settingsHeader.settingsLogoCustomPlaceholder);
				$(contentContainer).find('#logoTargetInput').attr("placeholder", i18n.builder.settingsHeader.settingsLogoCustomTargetPlaceholder);
				$(contentContainer).find('p').eq(1).html(i18n.builder.settingsHeader.settingsLogoSocialExplain);
				$(contentContainer).find('.control-label[for=selectSocialText]').html(i18n.builder.settingsHeader.settingsLogoSocialText + ":");
				$(contentContainer).find('.control-label[for=selectSocialLink]').html(i18n.builder.settingsHeader.settingsLogoSocialLink + ":");
				$(contentContainer).find('.header-share-label').html(i18n.builder.builder.buttonShare);
			}
		}
	}
);