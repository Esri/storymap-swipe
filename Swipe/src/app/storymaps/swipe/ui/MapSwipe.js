define(["dojo/dnd/move",
		...
		mouse)
	{
		return function MapSwipe(container)
		{
			...
			this.reposition = function(cfg)
			{
				if(app.mode == 'TWO_LAYERS'){
					if(app.sliderDiv){
						app.sliderDiv.node.style.left = $('#mainMap0').width() / 2 + _xOffset + "px";
						clipLayer(app.sliderDiv);
					}
				} else{
					$('#resizeWrapper').css('width', $('#mainMap0').width()/2 + _xOffset + "px");
				}
			}
