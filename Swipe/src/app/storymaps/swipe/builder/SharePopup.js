	...
	function (Helper) {
		return function SharePopup(container)
		{
			...
			{	
				...
				container.find('.first-save .header').html(i18n.swipe.share.firstSaveHeader);
				container.find('.first-save .question1').html(i18n.swipe.share.shareQ2bis);
				container.find('.first-save .answer1').html(
					i18n.swipe.share.firstSaveA1
					...
					+ i18n.swipe.share.firstSaveA1bis
						.replace('%LINK2%', contentUrl)
				);
				container.find('.first-save .question2').html(i18n.swipe.share.firstSaveQ2);
				container.find('.first-save .answer2').html(i18n.swipe.share.firstSaveA2);
				...
				if (app.mode == "TWO_WEBMAPS") {
					...
					if (app.data.getWebMapItem2().item.owner != app.portal.getPortalUser().username) {
						...
						if (app.data.getWebMapItem2().item.access == "account") {
							...
							if (app.data.getWebMapItem2().item.access == "private") {
								sharingStatus = i18n.swipe.share.shareWarningWith2;
								container.find('.not-shared .btn-sharePublic, .not-shared .btn-shareOrga').addClass("disabled").off('click');
							}
						...
						/*if( sharingStatus && map1Private ) {
	 						container.find('.share-warning').html(
	 							i18n.swipe.share.shareWarning
									.replace('%WITH%', sharingStatus)
 									.replace('%LINK%', webmapUrl)
	 								.concat("<a href='%LINK2%' target='_blank'>webmap</a>")
									.replace('%LINK2%', webmap2Url)
	 						).show();
	 					}*/
						if( sharingStatus && map1Private ) {
	 						container.find('.share-warning').html(
			...
			function presentShared(appUrl, itemUrl, webmapUrl, /*fsUrl,*/ contentUrl)
			{
				...
				container.find('.shared-question1')
					.html(
						'<strong>' 
						+(app.data.isOrga() ?
							i18n.swipe.share.shareQ1Opt1
							: i18n.swipe.share.shareQ1Opt2)
						+ '</strong>'
					).on('click', function() { 
						container.find('.shared-answer1').collapse('toggle');
					});
					
				container.find('.shared-answer1').html(i18n.swipe.share.shareA1
					.replace('%SHAREIMG%', '<img src="resources/icons/builder-share-shareBtn.png" style="vertical-align: -5px;"/>')
					.replace('%LINK1%', itemUrl)
					.replace('%LINK2%', webmapUrl)
					+ (app.data.sourceIsFS() && app.data.getFSSourceLayerItemId() ? ' ' + i18n.swipe.share.shareA1bis.replace('%LINK1%'/*, fsUrl*/) : '')
				);
				...
				container.find('.shared-question2')
					.html(
						'<strong>' 
						+ i18n.swipe.share.shareQ2bis
						+ '</strong>'
					).on('click', function() { 
						container.find('.shared-answer2').collapse('toggle');
					});
				container.find('.shared-answer2').html(i18n.swipe.share.shareA2div1
					...
					+ '<div style="margin-top: 3px">' + i18n.swipe.share.shareA2div2 + '</div>'
				...
				container.find('.shared-question3')
					.html(
						'<strong>' 
						+ i18n.swipe.share.shareQ3
						+ '</strong>'
					).on('click', function() { 
						container.find('.shared-answer3').collapse('toggle');
					});
				container.find('.shared-answer3').html(i18n.swipe.share.shareA3
